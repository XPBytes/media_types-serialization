...
class MediaTypes::ObjectWrapperTest < Minitest::Test
  ...
  def test_it_serializes_one
    ...
    assert_equal({"name"=>"first", "number"=>1, "items"=>[]}, result)
  end

  def test_it_extracts_the_links
    content_type = MyResourceMediaType.to_constructable.version(1).to_s
    Mime::Type.register(content_type, :my)

    do_request_chain(content_type)

    assert_equal("</item>; rel=self, <https://google.com>; rel=google; foo=bar", @response['Link'])
  end

  def test_it_coerces_data_into_a_single_object
    content_type = MyResourceMediaType.to_constructable.version(1).to_s
    Mime::Type.register(content_type, :my)
